...
EOF
    ...
    # JWT_SECRET is generated here, never templated: the backend refuses to boot
    # on a placeholder or a secret shorter than 32 chars.
    GENERATED_JWT_SECRET="$(openssl rand -hex 64)"
    cat > "$ENV_FILE" << EOF
# JWT Configuration (generated by deploy-railway.sh — keep secret, do not commit)
